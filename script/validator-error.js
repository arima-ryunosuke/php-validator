 ...
 * エラー表示用の jQuery 拡張
 *
 * エラー表示として上記の toastr を使用する。
 * validated さえ定義してあればよいので、比較的楽に表示方法を挿げ替えることができる。
 * ちなみに toastr はちょっと改造して onclick で閉じないようにしてある。
 */
(function ($) {
    // 基本的に validated イベントを listen することだけが唯一の仕事

    // エラー
    $(document).on('validated', ':input', function (e) {
        toast.call(this, e.detail.errorTypes, 'error');
    });

    // 警告
    $(document).on('mild-validated', ':input', function (e) {
        toast.call(this, e.detail.errorTypes, 'warning');
    });

    // DOM が消え去ったときに toast も消えるように監視する
    setInterval(function () {
        $('#' + toastrOptions.containerId).children().each(function () {
            var $this = $(this);
            var vinput = $this.data('vinput');
            if (vinput && vinput.closest('body').length === 0) {
                $this.remove();
            }
        });
    }, 111);
    ...
    // 指定要素までスクロールして目立たせる
    function scrollAndBlink($this) {
        while ($this.length && !$this.is(':visible')) {
            $this = $this.parent();
        }

        $this.addClass('validatable_blink');
    ...
    function toast(errors, type) {
        ...
        if (message.length) {
            var toast = $input.data(TOAST_NAME) || Chmonos.toastr[type](' ', $input.data('validation-title') || '', $.extend(true, {
                ...
                onHidden: function () {
                    $input.removeData(TOAST_NAME);
                },
            }, toastrOptions));
            toast.find('.toast-message').html(message.join('\n'));
            toast.data('vinput', $input);
            $input.data(TOAST_NAME, toast);
        }
        else {
            var toast = $input.data(TOAST_NAME) || $();
            toast.remove();
            $input.removeData(TOAST_NAME);
        }
    }
